...
Как упоминалось выше, после формирования множеств $P_{\alpha_j}$ мы можем продолжать работать с полигонами $p_i \subset \mathbb{R}^3$. Как и раньше будем считать, что $P_{\alpha_j} = \set{ p_i ~ | ~ \angle(\vec{n}_i, Oy) = \alpha_j}$. Теперь для выделения компонент связности внутри $P_{\alpha_j}$ необходимо определить отношение смежности. Естественным кажется выбор одного из двух определений: 

1) полигоны $p_i$, $p_j$ смежны, если существует вершина $v: ~ v \in p_i, ~ v \in p_j$;

2) полигоны $p_i$, $p_j$ смежны, если существуют вершины $u, v: ~ u, v \in p_i, ~ u, v \in p_j$.

Тогда после выделения компонент связности вся сетка $M$ разбивается на классы эквивалентности $\mathcal{C}^i_{\alpha_j}$.
