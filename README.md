# Поиск участка резьбы на 3D-модели
...
<p float="left">
  ...
  Остальные шаги в целом повторяют действия, описанные в предыдущем алгоритме. Мы должны объединить все компоненты $\mathcal{C}^i_{\alpha_j}$, соответствующие участку резьбы, в одно множество $\mathcal{S}$ и найти его границы.
  
  ### Примечание
  
  Для реализации этого подхода помимо прочего необходимо реализовать функцию $d(\mathcal{C}^i_{\alpha_j}, \mathcal{C}^l_{\alpha_j})$, вычисляющую расстояние (в смысле меру похожести) между двумя подмножествами из $\mathbb{R^3}$. 
  
  Отдельной задачей можно считать эффективное вычисление компонент связности. В случае алгоритма на изображении была использована функция `cv2.findContours()`. Однако, когда мы работаем с сеткой, нам скорее всего потребуется реализовать поиск компонент самостоятельно. Поиск компонет связности в графе задача известная и решается довольно быстро. Однако представление сетки как графа может оказаться трудоемкой (с точки зрения вычислений) задачей. 
